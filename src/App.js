...
export default function App() {
    ...
    //the SVG being displayed with default being the ethereum logo
    ...
    //list of svgs with default of only the ethereum logo
    const [svgNameList, setSVGNameList] = useState([
        "Ethereum Logo",
    ]);
    
    //update the svg being shown
    const changeCurrentSVG = (selectionTitle) => {
        //push previous svg to gun
        gun.get(svg.title).put({
            title: svg.title,
            svg: svg.svg,
        });
        
        //get new svg from selection
    ...
    //updates svg list with new svg
    const updateSVG = (newSVG) => {
        //updating gun database
        gun.get(newSVG.title).put({
            title: newSVG.title,
            svg:newSVG.svg,
        });

        //update list of svg names
